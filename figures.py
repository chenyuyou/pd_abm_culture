...
PARAMS = {
    ...
    # Steps at the end to average over (used by parallel runner)
    ...
    # --- Snapshot Specific ---
    # Example b values for snapshots
    'snapshot_b_values': [1.2, 1.6, 1.8, 2.2],
...
# Separate dir for snapshot files
...
COLOR_PALETTE_BASELINE = sns.color_palette(
    "viridis", n_colors=N_BASELINE_TYPES)
...
def generate_baseline_configs():
    ...
    sorted_c_items = sorted(
        PARAMS['baseline_C_values'].items(), key=lambda item: item[1])
    ...
    for label, c_val in sorted_c_items:
        for b in PARAMS['b_values']:
            ...
            for i in range(PARAMS['runs_per_config']):
                ...
                run_params['run_id'] = i
                # Assign unique seed
                run_params['seed'] = total_configs_generated
                # Store the descriptive label (e.g., 'C=0.1')
                run_params['label'] = label
                ...
                try:
                    ...
                    print(
                        f"Error creating SimConfig (Baseline). Params: {run_params}\nError: {e}")
...
def generate_heterogeneous_configs():
    ...
    for b in PARAMS['b_values']:
        ...
        for i in range(PARAMS['runs_per_config']):
            ...
            run_params['param_set_id'] = f"{param_set_id_base}_run{i}"
            run_params['run_id'] = i
            run_params['seed'] = seed_offset + \
                total_configs_generated  # Unique seed
            try:
                configs.append(SimConfig(**run_params))
                total_configs_generated += 1
            except TypeError as e:
                print(
                    f"Error creating SimConfig (Hetero). Params: {run_params}\nError: {e}")

    print(
        f"Generated {len(configs)} SimConfig objects for heterogeneous scan.")
    return configs
        ...
        try:
            ...
            if not isinstance(results_df, pd.DataFrame):
                ...
                if 'run_id' not in results_df.columns or 'label' not in results_df.columns:
                    print(
                        "Warning: 'run_id' or 'label' missing from loaded data. Consider rerunning.")
...
def save_plot(fig, base_filename, plot_dir=PLOT_SAVE_DIR):
    ...
    try:
        fig.savefig(
            png_path, dpi=mpl.rcParams['savefig.dpi'], bbox_inches='tight')
...
def plot_fig1(baseline_data, filename_base="fig1_baseline_coop_vs_b"):
    ...
    hue_order = sorted(baseline_data['label'].unique(
    ...
    ax.legend(title='Culture Type (Fixed C)',
...
def plot_fig4(baseline_data, hetero_data, filename_base="fig4_hetero_vs_homo_coop"):
    ...
    # Adjust as needed from PARAMS['baseline_C_values']
    baseline_labels_to_plot = ['C=0.0', 'C=0.4', 'C=1.0']
    baseline_subset = baseline_data[baseline_data['label'].isin(
        baseline_labels_to_plot)].copy()

    hetero_data_copy = hetero_data.copy()
    hetero_label = 'Heterogeneous'  # Get label from data
    combined_df = pd.concat(
        [baseline_subset, hetero_data_copy], ignore_index=True)

    # Define markers and dash styles dynamically
    unique_labels = sorted(combined_df['label'].unique(), key=lambda x: (
        x != hetero_label, float(x.split('=')[1]) if '=' in x else -1))
    n_unique = len(unique_labels)
    # Assign hetero color last
    palette = sns.color_palette(
        "viridis", n_colors=n_unique-1) + [COLOR_HETERO]
    markers = {label: m for label, m in zip(
        unique_labels, ['o', '^', 'D', 'v', 's'][:n_unique])}
    dash_styles = {label: (4, 1.5) if label ==
                   hetero_label else "" for label in unique_labels}
...
def plot_fig5(hetero_data, filename_base="fig5_group_coop_vs_b"):
    ...
    sns.lineplot(data=plot_df_melted, x='b', y='CooperationRate', hue='Group Type',
                 # Use a diverging palette (e.g., blue/red)
...
def run_and_save_snapshot(b_value, filename_tag="hetero"):
    ...
    snapshot_filename = os.path.join(
        SNAPSHOT_DATA_DIR, f"snapshot_{filename_tag}_b{b_value:.2f}.pkl")
    ...
    model_params = {
        ...
        'K_C': PARAMS['hetero_K_C'], 'p_update_C': PARAMS['hetero_p_update_C'], 'p_mut': PARAMS['hetero_p_mut'],
        # Unique-ish seed
        'seed': int(time.time() * 1000 + b_value * 100) % (2**32 - 1)
    }
...
def plot_fig2_snapshot(snapshot_data_file, filename_base_prefix="fig2_snapshot"):
    ...
    try:
        ...
        # {1: 'Group A (C<0.5)', 2: 'Group B (C>=0.5)'}
        type_desc = snapshot_data['types']
        L = params['L']
        b_val = params['b']
    except Exception as e:
    ...
    if TYPE_A_ID in type_desc:
        legend_labels[(TYPE_A_ID, STRAT_D)
                      ] = f"{type_desc[TYPE_A_ID]} / Defect"
        legend_labels[(TYPE_A_ID, STRAT_C)
                      ] = f"{type_desc[TYPE_A_ID]} / Cooperate"
    if TYPE_B_ID in type_desc:
        legend_labels[(TYPE_B_ID, STRAT_D)
                      ] = f"{type_desc[TYPE_B_ID]} / Defect"
        legend_labels[(TYPE_B_ID, STRAT_C)
                      ] = f"{type_desc[TYPE_B_ID]} / Cooperate"
    ...
    valid_legend_keys = sorted(
        [k for k in color_map if k != 'default' and k in legend_labels])
...
if __name__ == "__main__":
    ...
    baseline_raw_data = run_simulation_batch(
        generate_baseline_configs, BASELINE_DATA_FILENAME, force_rerun=FORCE_RERUN_BASELINE)
    ...
    if not baseline_raw_data.empty and 'label' in baseline_raw_data.columns:
        try:
            ...
            baseline_raw_data['c_value_sort'] = baseline_raw_data['label'].str.extract(
                r'C=([\d.]+)').astype(float)
            baseline_raw_data = baseline_raw_data.sort_values(
                'c_value_sort').drop(columns=['c_value_sort'])
        except Exception as e:
            print(
                f"Warning: Could not sort baseline data by label C value: {e}")

    # --- 2. Run/Load Heterogeneous Scan ---
    print("\n--- Processing Heterogeneous Data ---")
    hetero_raw_data = run_simulation_batch(
        generate_heterogeneous_configs, HETERO_DATA_FILENAME, force_rerun=FORCE_RERUN_HETERO)
    ...
    for b_snap in PARAMS['snapshot_b_values']:
        if FORCE_RERUN_SNAPSHOTS:
            snap_filename_temp = os.path.join(
                SNAPSHOT_DATA_DIR, f"snapshot_hetero_b{b_snap:.2f}.pkl")
            if os.path.exists(snap_filename_temp):
                try:
                    ...
                    print(
                        f"Error removing existing snapshot {snap_filename_temp}: {e}")

        # Snapshots use heterogeneous parameters by default in the function
        # Pass b value, tag is fixed
        fname = run_and_save_snapshot(b_snap, "hetero")
        if fname:
    ...
    if not snapshot_files:
        ...
        for snap_file in snapshot_files:
            # Uses default filename prefix "fig2_snapshot"
            plot_fig2_snapshot(snap_file)
            plot_count += 1
